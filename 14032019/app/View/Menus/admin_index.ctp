...
<div class="box-title">
	...
	<div class="col-sm-6">	<h3>Menus</h3></div>	<div class=" col-sm-6 addplus"  >
		<a href="<?php echo $this->webroot;?>admin/menus/add">
			<i class="fa fa-plus-circle" style="font-size:30px; color:#fff"></i>
		</a>
	</div>
</div>
<div class="box-content nopadding">
<table class="table table-hover table-nomargin table-bordered dataTable dataTable-column_filter" data-column_filter_types="null,text,select,text,null,null,null" data-column_filter_dateformat="dd-mm-yy"  data-nosort="0" data-checkall="all">
<thead>
<tr>
...
<tbody>
	...
	$men = $this->requestAction('/menus/view/'.$menu['Menu']['parent']);
	$name=$men['Menu']['name'];
	}
	?>
	<tr>
		<td><?php echo h($menu['Menu']['id']); ?>&nbsp;</td>
		<td><?php echo h($menu['Menu']['name'][$lang]); ?>&nbsp;</td>
		...
		<td><?php //echo h($menu['Menu']['usercreated']); ?>&nbsp;
        <?php //echo h($infosite['Infosite']['usercreated']);
		$userab = $this->requestAction('/users/affiche_user/'.$menu['Menu']['usercreated']);
		echo $userab['User']['name']; ?>
        </td>
		<td> &nbsp;
         <?php  $phota = $this->webroot.'files/menus/'.h($menu['Menu']['photo']) ;
		$photb = 'files/menus/'.h($menu['Menu']['photo']) ;
