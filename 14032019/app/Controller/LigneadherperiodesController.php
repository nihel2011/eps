...
if($idp==null)
		...
		$this->Ligneadherperiode->locale=Configure::read('Config.languages');
		  ...
		  $ligneadherperiodes = $this->Ligneadherperiode->findAll('all',array('conditions'=>array('Ligneadherperiode.etat'=>'Y','Ligneadherperiode.adherperiodes_id' =>$idp,'Ligneadherperiode.type'=>'A'),'order' => array('Ligneadherperiode.orderm DESC')));
	...
	public function comites($idp = null) {
/*		$this->Ligneadherperiode->recursive = 0;
		$this->set('ligneadherperiodes', $this->Paginator->paginate());
*/		
if($idp==null)
		...
		$this->Ligneadherperiode->locale=Configure::read('Config.languages');
		  ...
		  $ligneadherperiodes = $this->Ligneadherperiode->findAll('all',array('conditions'=>array('Ligneadherperiode.etat'=>'Y','Ligneadherperiode.adherperiodes_id' =>$idp, 'Ligneadherperiode.type'=>'C'),'order' => array('Ligneadherperiode.orderm DESC')));
 ...
 */
	public function admin_add() {
     $this->layout = 'admin';
		...
		$adherent = $this->Adherent->find('list',array('fields'=>'nom'));
		//print_r($categorie_id);
		
		$this->set(compact('adherent'));
		
		$this->loadModel('Adherperiode');
		$adherperiode = $this->Adherperiode->find('list',array('fields'=>'name'));
		//print_r($categorie_id);
		
		$this->set(compact('adherperiode'));
	...
	public function admin_edit($id = null) {
	 $this->Ligneadherperiode->locale=Configure::read('Config.languages');
		...
		if ($this->request->is(array('post', 'put'))) {
			...
			$options = array('conditions' => array('Ligneadherperiode.' . $this->Ligneadherperiode->primaryKey => $id));
			$this->request->data = $this->Ligneadherperiode->findAll('first', $options);
		}
